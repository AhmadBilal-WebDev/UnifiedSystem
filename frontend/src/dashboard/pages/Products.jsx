        ...
        {items.map((row, i) => (
          ...
          >
            ...
            <button
              type="button"
              className="btn btn-ghost"
              style={{ fontSize: 11, color: "var(--red)", padding: "7px 10px" }}
              onClick={() => removeRow(i)}
            >
              ✕
            </button>
...
export default function Products() {
  ...
  const hasMultipleBranches =
    (currentUser?.branchIds?.length || 0) > 1 || (branchList?.length || 0) > 1;
  ...
  return (
    <div className="page-content">
      ...
      >
        <div>
          ...
          <p style={{ fontSize: 12, color: "var(--text-m)", marginTop: 2 }}>
            {prods.filter((p) => p.active).length} active ·{" "}
            {prods.filter((p) => !p.active).length} inactive · {prods.length}{" "}
            total
          </p>
        </div>
      ...
      >
        ...
        <span
          style={{ fontSize: 12, color: "var(--text-m)", marginLeft: "auto" }}
        >
      ...
      <div className="grid3">
        {filtered.map((p, i) => {
          const margin =
            p.cost > 0 ? Math.round(((p.price - p.cost) / p.price) * 100) : 0;
              ...
              >
                ...
                <div style={{ flex: 1 }}>
                  ...
                  >
                    ...
                    <span
                      className={`badge badge-${p.active ? "green" : "red"}`}
                      style={{ fontSize: 9 }}
                    >
                      {p.active ? "Active" : "Off"}
                    </span>
      ...
      {showModal && (
        ...
        >
          <div className="grid-form-2">
            ...
            <OptionListEditor
              label="Sizes"
              hint="e.g. Small / Medium / Large, each with its own price"
              items={form.sizes}
              onChange={(v) => setForm((f) => ({ ...f, sizes: v }))}
            />
            <OptionListEditor
              label="Addons"
              hint="extra items the customer can add (e.g. Extra Patty)"
              items={form.addons}
              onChange={(v) => setForm((f) => ({ ...f, addons: v }))}
            />
            <OptionListEditor
              label="Extras"
              hint="free or paid extras (e.g. Extra Cheese, Ketchup)"
              items={form.extras}
              onChange={(v) => setForm((f) => ({ ...f, extras: v }))}
            />
                ...
                {[
                  ...
                  <button
                    ...
                    style={{
                      ...
                      cursor: "pointer",
                      border: "1px solid",
                      borderColor: form.tags.includes(t)
                        ? "var(--accent)"
                        : "var(--border)",
                ...
                <div>
                  ...
                  >
                    {form.applyToAllBranches
                      ? "This product will appear on every branch."
                      : "This product will only appear on your current branch."}
                  </div>
                </div>
                <label className="toggle">
                  <input
                    ...
                    onChange={(e) =>
                      setForm((f) => ({
                        ...f,
                        applyToAllBranches: e.target.checked,
                      }))
                    }
                  />
                  <span className="toggle-slider" />
